    ...
    <div className={`${className}`}>
      <div className="shell">
        <FadeIn className="shell-prose">
          <div className="bg-surface rounded-lg px-7 py-7 md:px-9 md:py-8">
            {label && (
              <div className="flex items-center gap-1.5 font-mono text-[11px] uppercase tracking-[0.14em] text-accent mb-3">
                {Icon && <Icon size={14} strokeWidth={1.75} aria-hidden />}
                <span>{label}</span>
              </div>
            )}
            <div className="text-[1.125rem] leading-[1.65] text-fg">{children}</div>
          </div>
        </FadeIn>
      </div>
    </div>
